...
def publish_live_catalog(home: Path | str, server: str, config_fingerprint: str,
    ...
    for record in records_list:
        for resource in record["resources"]:
            owners.setdefault(resource["uri"], []).append(record)
    for colliding in owners.values():
        if len(colliding) > 1:
            for record in colliding:
                record["metadata_allowed"] = False
                record["ownership_ambiguous"] = True
...
def _load_snapshot_locked(home: Path | str, session_id: str) -> tuple[dict[str, Any], ...] | None:
    try:
        ...
        if not isinstance(rows, list):
            return None
        records = tuple(_validate_record(row, connected=False) for row in rows)
        owners: dict[str, list[dict[str, Any]]] = {}
...
def mark_get_verified(record: dict[str, Any], home: Path | str | None, session_id: str | None) -> None:
    ...
    key = (home_key, session_id)
    with _lock:
        rows = list(_pin_session_all(home_key, session_id))
        matched = False
        for row in rows:
            if (row["server"], row["uri"], row["manifest_fingerprint"]) == (
                record["server"], record["uri"], record["manifest_fingerprint"]):
                row["get_verified"] = True
                matched = True
        if not matched:
            raise KeyError("remote skill is not present in the pinned session manifest")
        held = tuple(rows)
        _snapshots[key] = held
        secure_atomic_json(home_key, _snapshot_rel(session_id), {
            "schema_version": _SCHEMA_VERSION, "profile_key": _profile_key(home_key),
            "session_id_hash": _session_token(session_id), "entries": list(held)}, mode=0o600)
...
def continue_session(home: Path | str | None, parent_session_id: str, child_session_id: str) -> None:
    ...
    with _lock, ExitStack() as stack:
        for lock_id in lock_ids:
            stack.enter_context(secure_file_lock(home_key, _active_lock_rel(lock_id)))
        parent_snapshot = _snapshots.get(parent_key)
        if parent_snapshot is None:
            parent_snapshot = _load_snapshot_locked(home_key, parent_session_id)
        if parent_snapshot is not None:
            child_snapshot = tuple(json.loads(json.dumps(row)) for row in parent_snapshot)
            _snapshots[child_key] = child_snapshot
            secure_atomic_json(home_key, _snapshot_rel(child_session_id), {
                "schema_version": _SCHEMA_VERSION, "profile_key": _profile_key(home_key),
                "session_id_hash": _session_token(child_session_id), "entries": list(child_snapshot)}, mode=0o600)
        ...
        materialized_locks = [stack.enter_context(
            secure_file_lock(home_key, _materialized_lock_rel(lock_id))) for lock_id in lock_ids]
        if materialized_locks:
